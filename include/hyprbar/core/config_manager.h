...
struct ConfigValue {
    ...
    explicit ConfigValue(const std::string& s) : type(Type::String), string_value(s), int_value(0), double_value(0.0), bool_value(false) {}
    explicit ConfigValue(int64_t i) : type(Type::Integer), int_value(i), double_value(0.0), bool_value(false) {}
    explicit ConfigValue(double d) : type(Type::Double), double_value(d), int_value(0), bool_value(false) {}
    explicit ConfigValue(bool b) : type(Type::Boolean), bool_value(b), int_value(0), double_value(0.0) {}
    explicit ConfigValue(const std::map<std::string, ConfigValue>& obj) : type(Type::Object), object_value(obj), int_value(0), double_value(0.0), bool_value(false) {}
    explicit ConfigValue(const std::vector<ConfigValue>& arr) : type(Type::Array), array_value(arr), int_value(0), double_value(0.0), bool_value(false) {}
    ...
    const std::map<std::string, ConfigValue>& as_object() const { return object_value; }
    ...
    std::optional<ConfigValue> get(const std::string& key) const {
        ...
        if (it == object_value.end()) return std::nullopt;
        return it->second;
    }
};
