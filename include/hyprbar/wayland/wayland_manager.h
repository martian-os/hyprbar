 ...
 * 
 * Handles display connection, registry, compositor, shm, layer shell, and input.
 */
...
public:
    ...
    using PointerButtonCallback = std::function<void(uint32_t button, uint32_t state, int32_t x, int32_t y)>;
     ...
     * @return true on success
     */
    bool create_bar_surface(BarPosition position, uint32_t width, uint32_t height);
...
private:
    ...
    void configure_layer_surface(BarPosition position, uint32_t width, uint32_t height);
    // Wayland objects
    wl_display* display_;
