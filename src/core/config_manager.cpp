...
BarConfig::Position ConfigManager::parse_position(const std::string& pos) {
    ...
    if (lower == "top") return BarConfig::Position::Top;
    if (lower == "bottom") return BarConfig::Position::Bottom;
    if (lower == "left") return BarConfig::Position::Left;
    if (lower == "right") return BarConfig::Position::Right;

    return BarConfig::Position::Top;  // default
}

WidgetConfig::Position ConfigManager::parse_widget_position(const std::string& pos) {
    std::string lower = pos;
    std::transform(lower.begin(), lower.end(), lower.begin(), ::tolower);

    if (lower == "left") return WidgetConfig::Position::Left;
    if (lower == "center") return WidgetConfig::Position::Center;
    if (lower == "right") return WidgetConfig::Position::Right;
